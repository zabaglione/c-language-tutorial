#!/bin/bash
# pandocを使用して全章を統合したメインテキストと演習問題集を作成（最終版）
...
for chapter_info in "${CHAPTERS[@]}"; do
    ...
    # README.mdの内容を追加（タイトル行とサンプルコードセクション以降を除く）
    CHAPTER_DIR="${SRC_DIR}/${dir}"
    if [ -f "${CHAPTER_DIR}/README.md" ]; then
        # サンプルコードセクションの行番号を取得
        sample_line=$(grep -n "^## *サンプルコード" "${CHAPTER_DIR}/README.md" | head -1 | cut -d: -f1)
        
        if [ -n "$sample_line" ]; then
            # サンプルコードセクションの前までを抽出
            head -n $((sample_line - 1)) "${CHAPTER_DIR}/README.md" | tail -n +2 > /tmp/readme_content.md
        else
            # サンプルコードセクションがない場合は全体を使用
    ...
    # サンプルコードを追加（examples/ディレクトリから）
    if [ -d "${CHAPTER_DIR}/examples" ]; then
        echo "" >> "$TEMP_TEXTBOOK"
        echo "## サンプルコード" >> "$TEMP_TEXTBOOK"
        echo "" >> "$TEMP_TEXTBOOK"
        
        # ファイルをソートして追加（C99版を除く）
        for c_file in $(ls "${CHAPTER_DIR}/examples"/*.c 2>/dev/null | grep -v "_c99.c" | sort); do
            if [ -f "$c_file" ]; then
