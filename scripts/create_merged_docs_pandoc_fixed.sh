#!/bin/bash
# pandocを使用して全章を統合したメインテキストと演習問題集を作成
...
# 日付を動的に設定
sed -i '' "s/\$(date '+%Y年%m月%d日')/$(date '+%Y年%m月%d日')/g" "$TEMP_TEXTBOOK"
...
# 日付を動的に設定
sed -i '' "s/\$(date '+%Y年%m月%d日')/$(date '+%Y年%m月%d日')/g" "$TEMP_EXERCISES"
