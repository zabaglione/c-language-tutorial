#!/bin/bash
# pandocを使用して全章を統合したメインテキストと演習問題集を作成（コードブロック修正版）
...
# コードブロックの開閉を確認する関数
fix_unclosed_codeblocks() {
    local input_file="$1"
    local temp_file="/tmp/temp_fixed.md"
    
    # ファイルをコピー
    cp "$input_file" "$temp_file"
    
    # コードブロックの開始と終了をカウント
    local open_count=$(grep -c '^```' "$temp_file" || echo 0)
    
    # 奇数の場合は閉じられていないコードブロックがある
...
echo "pandocによる統合ファイル作成（コードブロック修正版）"
...
for chapter_info in "${CHAPTERS[@]}"; do
    ...
    if [ -f "${CHAPTER_DIR}/README.md" ]; then
        # 一時ファイルにREADMEをコピーして、コードブロックを修正
        TEMP_README="/tmp/temp_readme_${num}.md"
        tail -n +2 "${CHAPTER_DIR}/README.md" > "$TEMP_README"
        fix_unclosed_codeblocks "$TEMP_README"
        
        cat "$TEMP_README" >> "$TEMP_TEXTBOOK"
        echo "" >> "$TEMP_TEXTBOOK"
...
for chapter_info in "${CHAPTERS[@]}"; do
    ...
    if [ -f "${CHAPTER_DIR}/exercises/README.md" ]; then
        ...
        echo "" >> "$TEMP_EXERCISES"
        TEMP_EXERCISE="/tmp/temp_exercise_${num}.md"
        tail -n +2 "${CHAPTER_DIR}/exercises/README.md" > "$TEMP_EXERCISE"
        fix_unclosed_codeblocks "$TEMP_EXERCISE"
        cat "$TEMP_EXERCISE" >> "$TEMP_EXERCISES"
        echo "" >> "$TEMP_EXERCISES"
        rm -f "$TEMP_EXERCISE"
    fi
    ...
    if [ -d "${CHAPTER_DIR}/solutions" ]; then
        ...
        if [ -f "${CHAPTER_DIR}/solutions/README.md" ]; then
            TEMP_SOLUTION="/tmp/temp_solution_${num}.md"
            tail -n +2 "${CHAPTER_DIR}/solutions/README.md" > "$TEMP_SOLUTION"
            fix_unclosed_codeblocks "$TEMP_SOLUTION"
            cat "$TEMP_SOLUTION" >> "$TEMP_EXERCISES"
            echo "" >> "$TEMP_EXERCISES"
            rm -f "$TEMP_SOLUTION"
        fi
