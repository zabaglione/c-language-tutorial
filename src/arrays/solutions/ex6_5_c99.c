...
void display_all_words(void)
{
    ...
    for (int i = 0; i < word_count; i++) {  /* C99：forループ内変数宣言 */
...
void add_new_word(void)
{
    ...
    char new_english[MAX_WORD_LENGTH];  /* C99：必要な時に変数宣言 */
    ...
    char new_japanese[MAX_WORD_LENGTH];  /* C99：必要な時に変数宣言 */
