...
**注意**: 以下の表のサイズと範囲は一般的な値です。実際のサイズは使用するコンピューターやコンパイラーによって異なる場合があります。確実なサイズが必要な場合は`sizeof`演算子で確認してください。
...
コンパイラーが自動的に型を変換します。一般的に、小さい型から大きい型への変換は安全です。
...
プログラマーが意図的に型を変換する場合は、キャスト演算子を使います。
...
char * const ptr = buffer;          /* ポインター自体を変更不可 */
...
char *ptr = "Hello";      /* ポインター: 8バイト（64ビット環境） */
