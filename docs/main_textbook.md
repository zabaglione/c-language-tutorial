...
- [補章1: C言語の規格と歴史](#補章1-c言語の規格と歴史)
- [補章2: 開発環境の詳細設定](#補章2-開発環境の詳細設定)
...
int main(void)
{
    ...
    /* 学生データの追加 */
    strcpy(students[student_count].name, "田中太郎");
    students[student_count].age = ;
    students[student_count].gpa = .;
    student_count++;

    strcpy(students[student_count].name, "佐藤花子");
    students[student_count].age = 9;
    students[student_count].gpa = .;
    student_count++;

    strcpy(students[student_count].name, "鈴木一郎");
    students[student_count].age = ;
    students[student_count].gpa = .;
    student_count++;
    ...
    for (i = 0; i < student_count; i++)
        char *stats;
        if (students[i].gpa >= .) stats = stats_messages[];
        else if (students[i].gpa >= .) stats = stats_messages[];
        else if (students[i].gpa >= .) stats = stats_messages[];
        else stats = stats_messages[];

        printf("%s (%d歳) - GPA: %.f [%s]n",
               students[i].name, students[i].age, students[i].gpa, stats);
...
int main(void)
{
    ...
    printf("乗算: %d\n", math_func(5.0, 3.0)); /* multiply(, )を呼び出し */
    printf("除算: %.fn", calc_fnc(., .)); /* divide(., .)を呼び出し */

    /* 2つの異なる呼び出し方法 */
    printf("直接呼び出し: %d\n", (*math_func)(, ));  /* (*ptr)(args) */
    printf("間接呼び出し: %d\n", math_func(5.0, 3.0));     /* ptr(args) */
...
int main(void)
{
    ...
    int menu_size = sizeof(men) / sizeof(men[]);
    int choice;
    int i;
    ...
    do
        ...
        printf("選択してください (-%d): ", menu_size);

        if (scanf("%d", &choice) ==  && choice >=  && choice <= menu_size)
            printf("\n");
...
/* デバッグ用マクロ */
    ...
    #define DEBUG_PRINT(fmt, ...)
        fprintf(stderr, "[DEBUG %s:%d] " fmt "n", __IL__, __LIN__, ##__VA_ARGS__)
#else
    #define DEBUG_PRINT(fmt, ...) do  while()
#endif
...
/* 便利マクロ */
#define LOG_DEBUG(fmt, ...) logger_write(LOG_DEBUG, __IL__, __LIN__, fmt, ##__VA_ARGS__)
...
static const char *g_level_names[] =
    "DEBUG", "INO", "WARNING", "RROR", "CRITICAL"
;
...
void log_debug(const char *format, ...)
{
    ...
    va_list args;
    va_start(args, format);
    logger_write(LOG_DEBUG, NULL, , format, args);
    va_end(args);
...
SOURCS = $(SOURC_DIR)/math_utils.c
          $(SOURC_DIR)/string_utils.c
          $(SOURC_DIR)/logger.c
          $(SOURC_DIR)/config.c
...
HADRS = $(HADR_DIR)/math_utils.h
          $(HADR_DIR)/string_utils.h
          $(HADR_DIR)/logger.h
          $(HADR_DIR)/config.h
...
int main(void) {
    ...
    typeof(func_ptr) another_func = printf;  /* 同じ型の関数ポインター */
