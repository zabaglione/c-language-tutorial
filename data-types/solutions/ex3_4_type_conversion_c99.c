/*
 * ファイル名: ex3_4_type_conversion_c99.c
 * 演習3-4: 型変換の理解
...
int main(void)
{
    ...
    printf("\nC99固定幅整数型の変換:\n");
    int32_t i32 = 1000000;
    int64_t i64 = i32;  // 暗黙の拡張変換（安全）
    int16_t i16 = (int16_t)i32;  // 明示的な縮小変換（データ損失の可能性）
    
    printf("  int32_t %d → int64_t %lld (拡張変換)\n", i32, i64);
    printf("  int32_t %d → int16_t %d (縮小変換)\n", i32, i16);
    
    // より複雑な型変換の例
    printf("\n複雑な型変換の例:\n");
    uint32_t unsigned_val = UINT32_MAX;  // 最大値
    int32_t signed_val = (int32_t)unsigned_val;  // 符号付きに変換
    
    printf("  UINT32_MAX (%u) → int32_t (%d)\n", unsigned_val, signed_val);
...
6. C99固定幅整数型での安全な変換方法

C99の特徴:
- 固定幅整数型（int8_t, int16_t, int32_t, int64_t）
- 変数を使用箇所で宣言可能
- // 形式のコメント
